...
基于 `MiPlayAudioService.cmdSessionSuccess(...)` 的 APK 顺序，`DLNACast.Probe` 保留显式门控参数 `--miplay-native-safety-mutual-auth-local-device-info-probe=<IPv4>`。发送范围仍复用已验证的完整 SafetyAuth 互验路径，且只有在本端 `0x1402` 被设备 `0x1403` 验证、本端也验证设备 `0x1402 -> 0x1403` 后，才进入 post-auth device-info 阶段。

最初实现按 APK 上层调用顺序连续发送三帧：`0x001e getDeviceInfo`（seq `0x0004`）、两次 `0x0058 setLocalDeviceInfo`（seq `0x0005`/`0x0006`）。一次实机验证已对单台 `192.168.10.4` 执行。结果：完整 SafetyAuth 互验仍成功；随后三帧均按预期发出，长度分别为 `25`、`105`、`105`，但发送后未收到新的 post-auth follow-up frame，读 socket 时 Windows 返回 `SocketException 10053`。因此这次只能证明三帧已在认证后按预期发出，不能证明 S12 接受了 `0x0058` 或返回了 device-info。

在补齐 `0x001f` / `0x0059` 接收侧静态证据后，当前 probe 已改为 staged 行为：
...
新增的 `MiPlayAudioAccessUnitCipher` 只离线复刻上述 access-unit 加/解密形态：调用方显式提供受信任通道取得的 16 字节 `streamKey` / `streamIV`，加密时返回本次 access unit 的起始 IV 与 payload，并把下一次 IV 链接到最后一个完整密文块；解密时使用同一 CBC 链更新规则，保留尾部不足 16 字节的明文。它不生成、发现、传输或记录真实密钥，也不打包 MPEG-TS/RTP 或发送媒体。
...
- 旧式 `$` 控制帧的编码/解码和长度校验；
- `OpenDevice` 载荷、认证后 SafetyData 封装、RTSP 请求/响应诊断原语、RTP/MPEG-TS payload 边界原语、AAC access-unit 块加密原语、post-auth staged probe 门控策略、会话随机材料和只读 SafetyData 诊断辅助；
...
#### 2026-07-20 NetBus service Binder gates and ChannelServer registration evidence

This follow-up was also offline only: JADX Java sources and ARM64 ELF symbols/disassembly were read locally, with no S12 traffic. It narrows the SafetyAuth-success-to-`getDeviceInfo` gap further:

- `ContinuityConnectionManagerService.registerChannelListenerV2(...)` first records the Binder caller UID/PID, checks `PERMISSION_BIND_CONTINUITY_SERVICE_INTERNAL`, builds `AppInfo` with `PackageUtil.generateAppInfo(context, callingUid, callingPid, packageName)`, stores a `ChannelListenerServerProxy` in `mServiceListeners`, and only then calls `nativeRegisterChannelListener(serviceName.toMergeString(), appInfo, serverChannelOptionsV2, proxy)`.
- `createChannelV2(...)` has a parallel client-side gate: it requires `deviceId`, `ServiceName`, `ClientChannelOptionsV2`, listener, generated `AppInfo`, medium permission, internal bind permission, then calls `nativeCreateChannel(...)` and records the channel id in `ChannelMonitor`.
- `DeviceService.getDeviceInfo(...)` and `getServiceList(...)` are not raw socket commands. They pass `Binder.getCallingUid()` plus `deviceId` into `DeviceManagerNative.nativeGetDeviceInfo/nativeGetServiceList`, and return through `ResultReceiver`.
- `NetBusService.registerConnectionListener(...)` checks same-process, generates `AppInfo`, verifies medium permission, checks internal bind permission for the service id, then calls `NetBusManagerNative.nativeRegisterConnectionListener(Binder.getCallingUid(), serviceId, options, ServerConnectionListenerImpl, callback)`. `startConnectionServer(...)` similarly checks the service id permission before `nativeStartConnectionServer(Binder.getCallingUid(), serviceId, mediumType, callback)`. `requestConnection(...)` again requires package-derived `AppInfo` and service-id permission before `nativeRequestConnection(...)`.
- Native `ChannelManager::RegisterChannelListener` is only a short dispatch to `ChannelServer::RegisterChannelListener`. `ChannelServer::RegisterChannelListener` rejects duplicate server records, copies `ServerChannelOptions` and `AppInfo` into a `ServerRecord`, handles pending confirmations through `ChannelInitiationController`, and on the success path calls `ChannelServer::RegisterNetbusListener(serviceName, options)`. If `RegisterNetbusListener` fails, it erases the server record before returning the error.

Updated reproducible hypothesis: a successful 8899 SafetyAuth session plus an encrypted `0x001e` request is still missing at least one of the official Binder-side gates: caller UID/package-derived `AppInfo`, service-id permission context, registered server connection listener, started connection server, and `ServerChannelOptions`/medium state. Therefore, the absence of `0x001f` after `0x001e` remains expected static behavior rather than an unexplained frame-format failure. The next worthwhile offline target is to trace `ChannelServer::RegisterNetbusListener` through the generated callback/future path and map which `ServerChannelOptions` medium value for `miplay-audio` leads to WLAN/KCP versus other transports. No new S12 reprobe is justified yet.
#### 2026-07-20 RegisterNetbusListener medium mask and callback/future model

This offline pass traced `ChannelServer::RegisterNetbusListener` more tightly. The native function builds a server-side connection-options structure before calling the connection object obtained from `ChannelContext::GetConnection()`: the first word is `0x40083`, followed by `confirm=true`, then the `ServerChannelOptions` trust level, plus a copied optional-values tree. Using APK `MediumType.java`, `0x40083` decomposes to `BLUETOOTH | BLE | WIFI_LAN | REMOTE`; it does not include `WIFI_P2P` (`0x20`). APK `MediumType.getMainMediumType()` also collapses LAN-like values (`WIFI_HOTSPOT`, `WIFI_LAN`, `WIFI_LAN_1`, `WIFI_LAN_2`, `ETHERNET`, `WIFI_WLAN_ON_P2P`, `WIFI_HOTSPOT2`, `WIFI_WLAN_ON_WIFI_AWARE`) back to `WIFI_LAN`.

The same function allocates a promise/callback object and invokes the connection vtable with current process PID, service name, constructed server options, and callback state. If the native connection registration returns a non-zero error synchronously, the promise is completed with that error and the previously inserted server record is erased. This further supports the current model: official post-auth `getDeviceInfo`/listener success depends on a Binder/native listener-registration state machine and server connection options, not on sending one more legacy 8899 command after SafetyAuth.

The repository now has `MiPlayContinuityMediumTypes` and deterministic tests for the APK medium constants, WLAN-family collapsing behavior, and the native `RegisterNetbusListener` default mask. These tests are diagnostics only; they do not alter probe behavior or authorize any S12 network experiment.

#### 2026-07-20 RegisterNetbusListener vtable slot and static notifyConnect gates

This heartbeat pass remained offline only. No S12 connection was opened and no probe traffic was sent.

Native relocation evidence now pins the indirect call inside `ChannelServer::RegisterNetbusListener`: the connection object is obtained through `ChannelContext::GetConnection()`, its vptr is used with slot offset `+0x50`, and the `ConnectionManagerWrapper` vtable relocation at `0xF928E0` resolves to `lyra::netbus::conn::ConnectionManagerWrapper::RegisterConnectionListener(unsigned int, char const*, ServerConnectionOptions const&, ServerConnectionListener const&, ResultCallback const&)` at symbol address `0x94E278`. Because the ABI vptr points at `vtable + 0x10`, this matches `0xF92880 + 0x10 + 0x50 = 0xF928E0`. The argument setup immediately before the branch passes the current process PID, merged service name, native server connection options, listener callback, and result callback.

Java static-manager evidence adds another gate before a business service sees the connection. `NetworkingServiceParser` only builds `NetworkingServiceConfigInfo` from a service meta-data resource named `static_networking_service_list`, with `<networking_service_list><service ...>` attributes including `serviceName`, `notifyConnect`, `trustLevel`, `trustedTypes`, `needAddService`, and `serviceData`. `NotifyConnectHelper.onPackageLoaded(...)` registers a server-connection-initiation listener only when `notifyConnect` is true and the service passes `BIND_CONTINUITY_SERVICE_INTERNAL`; it builds `ServiceName(packageName, serviceName)`, stores a `ComponentKey(component, user, trustLevel)`, and calls `nativeRegisterServerConnectionInitiationListener(serviceName.toMergeString(), new ServerChannelOptionsV2(componentKey.trustLevel), this)`.

The listener trigger also has a trust comparison: `onConnectionInitiated(...)` ignores services that already have a server connection listener, parses the merged service name, looks up its `ComponentKey`, and only notifies the service when `connectionInfo.getTrustLevel() <= componentKey.trustLevel`; otherwise it rejects with `ERR_CODE_TRUST_LEVEL_TOO_LOW`. APK parsing maps `sameAccount -> 16`, missing/unknown/`trustGroup -> 32`, `sharedAccount -> 40`, and `everyOne -> 48`.

The current APK still does not expose a concrete `miplay-audio` static networking XML instance in its JADX resource tree. The visible `miplay-audio` string remains an IDM ServiceManager service type in `libidmservicemgr.so`, while the Continuity listener path expects Android package/service identity plus static config and Binder permission context. Test coverage now includes `MiPlayContinuityStaticNetworking`, `MiPlayContinuityServerConnectionOptions`, and `MiPlayContinuityNativeVtableEvidence`; these are diagnostics-only models and are not used by Probe sending code.

Updated testable hypothesis: the post-SafetyAuth `0x001e` silence is still more likely a missing official Continuity state problem than a single missing legacy-frame field. Specifically, the Windows Probe has not reproduced the Android package identity, static `notifyConnect` service config, trust-level acceptance gate, server connection initiation listener, or `RegisterConnectionListener` server state. Without new static evidence that maps those states back onto 8899 `0x001e`, another S12 read-only reprobe is not yet justified.

#### 2026-07-20 IDM miplay-audio service type versus Continuity ServiceName

This heartbeat pass was offline only. The APK zip was inspected locally, and no S12/device network operation was performed.

The packaged `AndroidManifest.xml` and `resources.arsc` entries do not contain `static_networking_service_list`, `networking_service_list`, `notifyConnect`, `trustLevel`, or `miplay-audio` in either UTF-8 or UTF-16LE form. That makes the previous static-manager conclusion sharper: this Mi Connect Service APK contains the parser and native listener APIs, but it does not itself publish a concrete static networking XML service instance for `miplay-audio`.
