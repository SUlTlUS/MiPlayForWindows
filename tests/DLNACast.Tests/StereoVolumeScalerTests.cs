...
public sealed class StereoVolumeScalerTests
{
    ...
    {
        var (Left, Right) = StereoVolumeScaler.ScaleToMaster(25, 11, 50);

        Assert.Equal(50, Left, precision: 6);
        Assert.Equal(22, Right, precision: 6);
    }

    [Fact]
    public void ScaleToMaster_ScalesDownFromTheLouderChannel()
    {
        var (Left, Right) = StereoVolumeScaler.ScaleToMaster(45, 90, 30);

        Assert.Equal(15, Left, precision: 6);
        Assert.Equal(30, Right, precision: 6);
    }

    [Fact]
    public void ScaleToMaster_UsesTheRequestedLevelWhenBothChannelsAreMuted()
    {
        var (Left, Right) = StereoVolumeScaler.ScaleToMaster(0, 0, 40);

        Assert.Equal(40, Left, precision: 6);
        Assert.Equal(40, Right, precision: 6);
    }
